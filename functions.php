...
function verberg_classes_met_js() {
    ?>
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            var elementsToHide = document.querySelectorAll('.user-rich-editing-wrap, ' +
...
add_action('admin_enqueue_scripts', 'verberg_classes_met_js');
...
// Verwijder de WordPress admin bar voor de gebruikersrol "Abonnee"
function verwijder_admin_bar_voor_abonnee() {
    if (current_user_can('partner')) {
        add_filter('show_admin_bar', '__return_false');
    }
}
add_action('after_setup_theme', 'verwijder_admin_bar_voor_abonnee');

// Doorverwijzen naar de homepagina na het inloggen voor de gebruikersrol "Abonnee"
function doorverwijzen_naar_home_voor_abonnee($redirect_to, $request, $user) {
    // Is de gebruiker ingelogd en heeft de rol "Abonnee"?
    if (isset($user->roles) && is_array($user->roles) && in_array('subscriber', $user->roles)) {
        return home_url(); // Doorverwijzen naar de homepagina
    }
    return $redirect_to; // Geen wijzigingen voor andere gebruikersrollen
}
add_filter('login_redirect', 'doorverwijzen_naar_home_voor_abonnee', 10, 3);
...
function redirect_to_login_page() {
    ...
    if (!is_user_logged_in()) {
        // Controleer of de huidige pagina niet de inlogpagina is om oneindige omleiding te voorkomen
        if (!is_page('login')) {
            // Stel de URL in van de inlogpagina
            $login_url = home_url('/wp-admin/'); // Vervang '/login/' door de daadwerkelijke URL van je inlogpagina
...
// Voeg de functie toe aan de actie 'template_redirect'
...
function customize_user_roles_dropdown($all_roles) {
    // Lijst van rollen die je wilt verwijderen
...
// Voeg de filter toe aan de 'editable_roles'-hook
add_filter('editable_roles', 'customize_user_roles_dropdown');
...
function wijzig_loginpagina_logo() {
    echo '<style type="text/css">
        .login h1 a {
            background-image: url(' . get_template_directory_uri() . '/img/local/Knauf-logo-2455482B1E-seeklogo.com.png) !important;
            width: 100% !important;
